...
/**
 * Get a mock bridge for the LedgerKeyring.
 *
...
function getMockBridge(): LedgerBridge<LedgerBridgeOptions> {
  return {
    ...
    attemptMakeApp: jest.fn().mockResolvedValue(true),
...
describe('LedgerKeyringV2', () => {
  ...
  it('constructs with expected type and capabilities', () => {
    expect(wrapper.type).toBe('ledger');
  ...
  describe('createAccounts', () => {
    ...
    it('throws when the legacy keyring does not return an address', async () => {
      ...
      await expect(wrapper.createAccounts()).rejects.toThrow(
        'Failed to create Ledger keyring account',
      );
    });
  });
  ...
  describe('submitRequest', () => {
    ...
    it('throws for unsupported method', async () => {
      ...
      await expect(wrapper.submitRequest(req)).rejects.toThrow(
        `Unsupported method for LedgerKeyringV2: ${method}`,
      );
    });
