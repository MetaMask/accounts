/**
 * Returns browser-side WebHID mock source for Speculos over WebSocket.
 *
  ...
  return `
    (function() {
      ...
      const connectWebSocket = function() {
        if (ws && (ws.readyState === _WS_OPEN || ws.readyState === _WS_CONNECTING)) {
          return;
        }
      ...
      const runHidExchange = function(device, frameData) {
        return new Promise(function(resolve, reject) {
          const startSend = function() {
            ...
            messageId += 1;
            const exchangeId = messageId;
            pendingExchanges.set(exchangeId, { resolve: resolve, reject: reject, device: device });
