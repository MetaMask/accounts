...
export class HdKeyring implements Keyring {
  ...
  async deserialize(
    ...
    if (opts.numberOfAccounts && !opts.mnemonic) {
      throw new Error(
        'Cash-Account-Keyring: Deserialize method cannot be called with an opts value for numberOfAccounts and no menmonic',
      );
    }

    if (this.root) {
      throw new Error(
        'Cash-Account-Keyring: Secret recovery phrase already provided',
      );
    }
