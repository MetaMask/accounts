...
import type { TrezorControllerState, TrezorKeyring } from './trezor-keyring';
 ...
 * Trezor keyrings support signing transactions, personal messages, and typed data (v3 and v4).
 */
const TREZOR_KEYRING_EOA_METHODS = [
...
export class TrezorKeyringV2
  extends KeyringWrapper<TrezorKeyring>
  implements KeyringV2
{
  ...
  #createKeyringAccount(address: Hex): KeyringAccount {
    ...
    const account: KeyringAccount = {
      ...
      methods: TREZOR_KEYRING_EOA_METHODS,
      options: {
        entropy: {
  ...
  async deserialize(state: Json): Promise<void> {
    ...
    await this.inner.deserialize(state as TrezorControllerState);
  ...
  async createAccounts(): Promise<KeyringAccount[]> {
    ...
    if (!address) {
      throw new Error('Failed to create Trezor keyring account');
    }
  ...
  async submitRequest(request: KeyringRequest): Promise<Json> {
    ...
    switch (method) {
      ...
      default:
        throw new Error(`Unsupported method for TrezorKeyringV2: ${method}`);
    }
  }
}
