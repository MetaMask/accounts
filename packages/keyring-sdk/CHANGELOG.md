...
- Add `getDeterministicUuidV4{,Random}` for deriving deterministic UUID v4 values from arbitrary hex strings ([#638](https://github.com/MetaMask/accounts/pull/638))
...
- Memoize `generateEthAccountId` ([#638](https://github.com/MetaMask/accounts/pull/638))
  - Following the `@metamask/utils` pattern for memoization (use of `lodash`'s `memoize`) .
- Replace `ethereum-cryptography` with `@noble/hashes` in the account ID derivation ([#638](https://github.com/MetaMask/accounts/pull/638))
